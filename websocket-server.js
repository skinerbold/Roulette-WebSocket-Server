...
async function ensureHistoryLength(rouletteId, limit) {
    ...
    // 🚀 CORREÇÃO: BYPASS DO SUPABASE - Buscar direto da API Fly.io
    console.log('⚡ BYPASS ATIVADO: Pulando Supabase, buscando direto da API Fly.io');
    
    try {
        const flyApiUrl = process.env.FLY_API_URL || 'https://roulette-api.fly.dev';
        console.log(`🌐 Buscando ${limit} números de: ${flyApiUrl}`);
        
        const apiNumbers = await fetchFromFlyApi(flyApiUrl, rouletteId, limit);
        
        if (apiNumbers && apiNumbers.length > 0) {
            console.log(`📦 API retornou ${apiNumbers.length} números`);
            
            // Verificar unicidade
            const uniqueNumbers = new Set(apiNumbers).size;
            console.log(`🔍 Números únicos: ${uniqueNumbers}/${apiNumbers.length}`);
            
            if (uniqueNumbers < 50 && apiNumbers.length >= 100) {
                console.error(`⚠️  ALERTA: Apenas ${uniqueNumbers} números únicos! Possível problema na API.`);
            }
            ...
            console.log(`✅ ${entries.length} números carregados e armazenados no cache`);
        } else {
            console.error('❌ API Fly.io não retornou números');
        }
    } catch (error) {
        console.error('❌ Erro ao buscar da API Fly.io:', error.message);
    }
} Servidor WebSocket para Roleta ao Vivo
// Refatorado para isolar histórico por roleta, persistir lançamentos
// e implementar o protocolo subscribe/unsubscribe/get_history.
...
// ============================================
// MAPEAMENTO DE NOMES DE ROLETAS
// ============================================
// Mapeia os nomes usados no frontend para os nomes aceitos pela API Fly.io
const ROULETTE_NAME_MAP = {
    // Evolution Gaming
    'speed auto roulette': 'speed-roulette',
    'lightning roulette': 'lightning-roulette',
    'instant roulette': 'instant-roulette',
    'first person roulette': 'first-person-roulette',
    'auto roulette': 'speed-roulette', // Auto Roulette da Evolution -> Speed Roulette
    'auto roulette vip': 'speed-roulette',
    
    // Pragmatic Play
    'american roulette': 'pragmatic-roulette',
    'mega roulette': 'pragmatic-mega-roulette',
    'power up roulette': 'pragmatic-powerup-roulette',
    'speed auto roulette pragmatic': 'pragmatic-speed-auto-roulette',
    'auto-roulette pragmatic': 'pragmatic-auto-roulette',
    
    // Mapeamentos diretos (já no formato correto)
    'speed-roulette': 'speed-roulette',
    'lightning-roulette': 'lightning-roulette',
    'instant-roulette': 'instant-roulette',
    'first-person-roulette': 'first-person-roulette',
    'pragmatic-auto-roulette': 'pragmatic-auto-roulette',
    'pragmatic-roulette': 'pragmatic-roulette',
...
async function fetchFromFlyApi(baseUrl, rouletteId, limit) {
    return new Promise((resolve, reject) => {
        // 🗺️ CORREÇÃO: Mapear nome da roleta para formato aceito pela API
        const normalizedName = rouletteId.toLowerCase().trim();
        const apiName = ROULETTE_NAME_MAP[normalizedName] || normalizedName;
        
        console.log(`🗺️ Mapeamento: "${rouletteId}" -> "${apiName}"`);
        
        const url = `${baseUrl}/api/history/${encodeURIComponent(apiName)}?limit=${limit}`;
